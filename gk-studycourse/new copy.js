...
(function () {
  window.onload = function () {
    ...
    var ActivitiesReadUrl = "https://lms.ouchn.cn/api/course/activities-read/$";
    /* 课程页面 */
    if (/lms.ouchn.cn\/course\/(\d+)/.test(href)) {
      setTimeout(async () => {
        ...
        let { completed_result: completed_result } = await totalOnlineVideo(
          page_courseid
        );
        let completeVideo = completed_result.completed.learning_activity;
        // 显示课程数
        showMsg(
        ...
        if (learning_activities != null || learning_activities !== undefined) {
          for (const item of learning_activities) {
            if (!completeVideo.includes(item.id)) {
              /*
              ...
              */
              switch (item.type) {
                ...
                case "online_video":
                  ...
                  let UserVisitHeader = Object.assign({}, tmpbody);
                  ...
                  if (uploads.length > 0) {
                    for (const media of uploads) {
                      if (media.type == "video") {
                        ...
                        online_video.method = "POST";
                        online_video.body = JSON.stringify(body);
                        online_video.referrer =
                          "https://lms.ouchn.cn/course/18068/learning-activity/full-screen";
                        allPromise.push(
                          {
                            url:
                              "https://lms.ouchn.cn/api/course/activities-read/$" +
                              item.id,
  ...
  function totalOnlineVideo(courseId) {
    return fetch(
      `https://lms.ouchn.cn/api/course/${courseId}/my-completeness`,
      {
        headers: {
          ...
          pragma: "no-cache",
          "sec-ch-ua":
            '"Not?A_Brand";v="8", "Chromium";v="108", "Brave";v="108"',
          "sec-ch-ua-mobile": "?0",
          "sec-ch-ua-platform": '"Windows"',
          "sec-fetch-dest": "empty",
          "sec-fetch-mode": "cors",
        ...
        referrer: `https://lms.ouchn.cn/course/${courseId}/ng`,
        referrerPolicy: "strict-origin-when-cross-origin",
        body: null,
        method: "GET",
        mode: "cors",
