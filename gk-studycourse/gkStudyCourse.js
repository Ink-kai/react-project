          ...
          learning_activities.forEach((item) => {
            ...
            switch (item.type) {
              ...
              case "online_video":
                ...
                let lockVideoBody = Object.assign({}, this.header);
                ...
                if (uploads.length > 0) {
                  uploads.forEach((media, i) => {
                    if (media.type == "video") {
                      ...
                      Promise.all([
                        this.activitiesRead(item.id, lockVideoBody),
                        /* 增加音视频查看次数 */
                        this.onlineVideo(this.header, online_video),
                        this.onlineVideo(this.header, online2_video),
                        /* 增加课程访问次数 */
                      ...
                      topic_categories.forEach((item, index) => {
                        ...
                        this.GetTopics(id, header1).then((Topicres) => {
                          ...
                          ExistsMyTopic = ExistsMyTopic.filter(
                            (item) =>
                              item.created_by.id == this.globalData.user.id
                          );
                          // 发过贴则不再发
                          if (ExistsMyTopic.length === 0) {
                            ...
                            let topic_header = Object.assign({}, header1);
                            /* 发帖内容 */
                            ...
                            let body = {
                              ...
                              channel: "web",
                              target_info: {},
                              master_course_id: $("#masterCourseId").val(),
                              topic_name: "好好学习",
                              org_name: this.globalData.user.orgName,
                              org_code: this.globalData.user.orgCode,
                              user_no: this.globalData.user.userNo,
...
<ul style="height: 550px;overflow-y: auto;"><li hidden></li></ul></div>`;
  ...
  */
      ...
      let answer = "";
      // 搜索答题接口
      await fetch(`http://127.0.0.1:6007/v1/GetAnswer?name=${question_keyword}`)
        .then((res) => res.json())
    ...
    function answerQuestion(item, answer, score, question_index, nullQuestion) {
      ...
      if (
        $(item).find("[name='ink-Answer'").length == 0 &&
        isChecked === false
      ) {
        $(item)
          .find(".summary-title")
  ...
  async getAllCourse() {
    ...
    let result = await fetch(
      `https://lms.ouchn.cn/api/course/${this.page_courseid}/all-activities?module_ids=[${this.moduleids}]&activity_types=learning_activities,exams,classrooms,live_records,rollcalls&no-loading-animation=true`,
      header
    );
